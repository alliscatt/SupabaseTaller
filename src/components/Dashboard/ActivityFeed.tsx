...
export function ActivityFeed({ tareas }: { tareas: Tarea[] }) {
  ...
  return (
    <div style={{ background:'white', borderRadius:'12px', padding:'1.5rem',
      ...
      <h3 style={{ margin:'0 0 1rem', fontSize:'1rem', color:'#2e2d2dff'}}>🕘 Actividad reciente</h3>
      {tareas.length===0
        ? <p style={{ color:'#000000ff' }}>Sin actividad</p>
        : tareas.map(t => (
          <div key={t.id} style={{ display:'flex', gap:'0.75rem',
            padding:'0.6rem', marginBottom:'0.4rem', borderRadius:'8px',
            background: t.completada ? '#2a3655ff' : '#2a3655ff',
            ...
            <div style={{ flex:1, overflow:'hidden' }}>
              ...
              <p style={{ margin:0, fontSize:'0.75rem', color:'#ffe7e7ff' }}>
