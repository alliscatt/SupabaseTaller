...
export const dashboardService = {
  ...
  getStats: async () => {
    ...
    const tareas = data as unknown as { completada: boolean; created_at: string }[]
    const total = tareas.length
    const completadas = tareas.filter(t => t.completada).length
    const pendientes   = total - completadas
    const porcentaje   = total > 0 ? Math.round((completadas / total) * 100) : 0
    const hoy          = new Date().toISOString().split('T')[0]
    const creadasHoy = tareas.filter(t => t.created_at.startsWith(hoy)).length
  ...
  getActivityByDay: async () => {
    ...
    const tareas = data as unknown as { created_at: string; completada: boolean }[]
